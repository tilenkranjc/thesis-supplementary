...
# load data
data<-data.frame()
for (i in list.files("length",pattern="p1.*.tab",full.names = TRUE)) {
  data<-rbind(data,cbind(read.table(i),data.frame(trt="siPRAF1")))
}

for (i in list.files("length",pattern="neg.*.tab",full.names = TRUE)) {
...
# calculate new means
means<-ddply(data,.(trt),summarise,mean=mean(Area),
             median=median(Area),
             sd=sd(Area),
             se=sd(Area)/sqrt(length(Area)))

# and calculate means in micrometers
means.um<-ddply(data,.(trt),summarise,mean=mean(Area)*scale1,
                median=median(Area)*scale1,
                sd=sd(Area)*scale1,
                se=sd(Area)*scale1/sqrt(length(Area)))

# correct factor names
means.um$trt<-factor(means.um$trt,levels=c("siNEG","siPRAF1"),ordered=TRUE)

# perform a t-test
t.test(Area~trt,data=data)
...
plot + theme_bw() + labs(title="Length of mitochondria\nin PRAF1 depleted cells",
                         x="Treatment", y=expression(paste("Mean length in ",mu,"m"))) + 
  scale_y_continuous(limits=limits, expand = c(0,0))+
  theme(text=element_text(size=text_points),plot.margin=unit(c(1,1,0,0),"mm"))

# save
ggsave(file="rnai-length.pdf",width=80,height=100,units="mm")
ggsave(file="rnai-length.tiff",width=80,height=100,units="mm")
